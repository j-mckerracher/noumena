...
Use Claude Code's `!` pre-execution syntax so the skill sees the active Azure DevOps CLI defaults before building commands.

```text
!`az devops configure --list 2>/dev/null`
```

Prefer the injected org and project defaults over asking the user to restate existing CLI configuration.
