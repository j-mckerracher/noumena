 ...
 *   - stale_review approve exits 4 (not 0)
 *   - rollback_not_latest exits 4 (not 0)
...
describe("patch approve exit codes", () => {
  it("exits 4 with stale_review when block hash changed since review was queued", () => {
    ...
    // 3. Approve the queued review — should fail with stale_review (exit 4)
    const approveResult = runCli([
      "patch", "approve", tmpDir, reviewId, "--json",
    ]);
    expect(approveResult.exitCode).toBe(4);
    const approveJson = JSON.parse(approveResult.stdout);
    expect(approveJson.error.code).toBe("stale_review");
  });
});

describe("patch rollback exit codes", () => {
  it("exits 4 with rollback_not_latest when rolling back a non-latest patch", () => {
    ...
    // 2. Try to rollback the first (non-latest) patch — should exit 4
    const rollbackResult = runCli([
      "patch", "rollback", tmpDir, patchId1, "--json",
    ]);
    expect(rollbackResult.exitCode).toBe(4);
    const rollbackJson = JSON.parse(rollbackResult.stdout);
    expect(rollbackJson.error.code).toBe("rollback_not_latest");
  });
});
