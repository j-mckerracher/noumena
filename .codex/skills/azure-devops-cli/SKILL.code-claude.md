...
Install Azure CLI and Azure DevOps extension IF AND ONLY IF it is not already installed:
...
### Important: Non-Interactive Environment

In Claude Code and automated environments, **never use bare `az devops login`** without piping credentials — it will block waiting for input. Always use one of the non-interactive methods below.

### Preferred: PAT via Environment Variable

The session has `AZDO_PAT` set. Use it to authenticate without any interactive prompt:

```bash
# Option A: pipe PAT directly (works every time, safe from shell history)
echo $AZDO_PAT | az devops login --organization https://dev.azure.com/{org}

# Option B: export as the native env var — no login command needed at all
export AZURE_DEVOPS_EXT_PAT=$AZDO_PAT
# All subsequent az devops commands will automatically use this PAT
```

Option B (exporting `AZURE_DEVOPS_EXT_PAT`) is the cleanest: the CLI picks it up natively and you don't need to call `az devops login` at all.

### Check Auth Before Acting

Before attempting login, verify whether the CLI is already authenticated:

```bash
if az devops project list --organization https://dev.azure.com/{org} --top 1 --output none 2>/dev/null; then
...
```bash
# Set default organization and project to avoid repeating --org and --project on every command
